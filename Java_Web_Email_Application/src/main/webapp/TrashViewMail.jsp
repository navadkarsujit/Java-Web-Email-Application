...
%>
	<div class="notes">
		<table border=0>
			...
			<tr>
				<td>
					<div class="title2">
						<label for="title"><label class="l1">From : </label><%=rs.getString("SENDER") %></label><br>
					</div>
					<hr>
				</td>
			</tr>
			...
			<tr>
				<td>
					<div class="title4">
						<button class="btn1">
							<a href='CompleteDelete.jsp?ID=<%= rs.getString(1)%>'>Delete
								Mail</a>
						</button>
						<button class="btn2">
							<a href='Undo.jsp?ID=<%= rs.getString(1)%>'>Undo</a>
						</button>
					</div>
				</td>
			</tr>
	...
	<!-- 		<p>Date <%=rs.getString("MESSAGEDATE") %></p>
		<p>Message :<%=rs.getString("MESSAGE") %></p>
		<p>Sender :<%=rs.getString("SENDER") %></p>
		<p>Subject :<%=rs.getString("SUBJECT") %></p>
		<button><a href='CompleteDelete.jsp?ID=<%= rs.getString(1)%>'>Delete Mail:</a></button>
 -->
	<%
	}
	conn.close();
